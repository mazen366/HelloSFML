...
int main()
{
	...
	Texture Tbackground, Tkey, Tcoin, button, Tplayer;
	Tbackground.loadFromFile("SFMLProject2.png");
	Tkey.loadFromFile("key2.png");
	Tcoin.loadFromFile("spinning_coin2.png");
	button.loadFromFile("button_ui.png");
	Tplayer.loadFromFile("Walking.png");

	Sprite background(Tbackground), key(Tkey, IntRect(0, 0, 39, 39)), coin(Tcoin, IntRect(0, 0, 26, 22)), player(Tplayer, IntRect(0, 0, 400 / 8, 180));
	background.setPosition(Vector2f(0, 250));
	key.setPosition(790, 555);
	coin.setPosition(1290, 590);
	player.setPosition(20, 680);
	...
	float xPlayer = 0, delayPlayer = 0.08;
	...
	Font font;
	font.loadFromFile("Nightmare Before Christmas.ttf");
	...
	while (window.isOpen())
	{
		float timer = time.getElapsedTime().asMicroseconds();
		time.restart();
		timer /=5900;
		...
		else if (started && !paused)
		{
			...
			{
				player.setScale(2, 2);
				//player.setOrigin(Vector2f(Tplayer.getSize().x / 2, Tplayer.getSize().y / 2));
				player.setTextureRect(IntRect(short int(xPlayer) * 400 / 8, 0, 400 / 8, 180));
				xPlayer += delayPlayer*timer;
				if (xPlayer > 7.9)
					xPlayer = 0;
				player.move(2*timer, 0);
			}
